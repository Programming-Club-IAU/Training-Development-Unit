
## Week 05: 📐 Front-end Intro

In this lecture, we diverged into the Front-end Path and explored Javascript Basics.


<br>

## 🎥 Lecture Recording

You can watch the recorded session here: [Watch the recording](https://udksa-my.sharepoint.com/:v:/g/personal/ccsit_pclub_iau_edu_sa/EZvAV19lyJBJulSNFEpEePcBI_xlyceJepiNC71MvDt8ZA?e=VSqYau&nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJTdHJlYW1XZWJBcHAiLCJyZWZlcnJhbFZpZXciOiJTaGFyZURpYWxvZy1MaW5rIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXcifX0%3D)

The slides used in the lecture:
[Slides](/assets/Front_01.pdf)


<br>

## 📚 Self-Learning Resources

As part of this course, you’re expected to review the following materials to strengthen your understanding in Javascript. **These are mandatory and crucial for your understanding**
These resources will help you understand more about the lecture topics *and* more:

- [Bro Code Javascript]( https://youtube.com/playlist?list=PLZPZq0r_RZOO1zkgO4bIdfuLpizCeHYKv&si=r2Wb4YM9CtmfL_Fj)
- [W3School - Javascript](https://youtu.be/H9zdfZrFUp0)
- [W3School - HTML Forms](https://www.w3schools.com/html/html_forms.asp)

<br>

**You are not expected to link Javascript with HTML at this level**, so as of right now we will use an online compiler to interperet our code. 
Suitable JS Online Compilers:
- [Programiz](https://www.programiz.com/javascript/online-compiler/)
- [OneCompiler](https://onecompiler.com/javascript)

<br>

## 🧠 Assignment

Use everything we learned in the session and self-learning, get creative 🦃

Note: You dont need to add input for the project since javascript usually gets the data from the html.
just show us a functioning project with ready data

*Hints* if you want ideas:
- Calculator
- Greeting every name in the array
- Reverse Array
- Fibonacci





<br>


## ✅ Submission

Once you’ve completed your task, share your repository link in the GitHub assignment form:
[Submission Form](https://forms.cloud.microsoft/Pages/ResponsePage.aspx?id=_LuGLASN_0GoOpQvB14PYB21Z8AvW0FIsQSmL58EU41UOEgxVUxMRzY4RzZLNFc5M1FRVk1ERDBWUC4u)

<br>

## 💡 Additional Resources (Optional)

If you’re interested in learning Javascript, here are some extra materials you can check out at your own pace:
- [Eloquent Javascript]( https://eloquentjavascript.net/)
- [TheOdinProject - Javascript]( https://www.theodinproject.com/paths/foundations/courses/foundations#javascript-basics) 
- [Javascript.info](javascript.info/#tab-1) 
- [Javascript Tutorial](https://www.javascripttutorial.net/) 
